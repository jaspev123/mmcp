import MCPDuckDBAgent from './duckdb-agent22';
  ...
  try {    
   ...
   let result: string = "";
    if (args.includes("ollama")) {
      ...
      result = await agent.callLLMLoop(taxi_trips_distance_per_month_query, agent.generateSQLWithBedrockLLM.bind(agent));
    }
    
    console.log("\n Final result for running the Query in Duckdb:\n","-----\n", result,"-- End of Queried data---\n");
  } catch (err) {
