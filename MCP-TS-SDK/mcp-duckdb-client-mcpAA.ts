...
import { callOllamaLLJson, callOllamaLLJsonTools, callOllamaLLM, ChatMessage, OllamaLLMResponse, OllamaLLMResponseJSON, OllamaLLMResponseToolsMessage, OllamaLLMTool } from "./ollamaLLM";
...
export class MCPDuckDBAgent {
  ...
  constructor() {
    ...
    this.mcpTransport = new StdioClientTransport({
      command: "tsx",
      args: ["mcp-duckdb-server-mcpAA"] // Path to your compiled MCP server
    });
  }
  ...
  async callLLMLoop(question: string,llmFunction: (q: ChatMessage[], tools: any[]) => Promise<any>): Promise<any> {
    ...
    try {      
        ...
        while (doMoreWorkWithLLM) {                                  
            ...
            for (const toolCall of response.message.tool_calls) {          
                ...
                const { toolResult, isError } = await this.mcpClient.callTool({
                  name: toolName,            
                  arguments: toolArgs
                });
                ...
                currentMessages.push({
                  role: "tool",
                  content: isError ? `Error: ${toolResult}` : toolResult
              });
   ...
   async generateSQLWithOllamaLLM(messages :ChatMessage[], tools: any[]): Promise<any> {    
    ...
    const result  : OllamaLLMResponseToolsMessage = await callOllamaLLJsonTools("llama3.2", messages, ollamaTools, this.ollmaContext);         
    return result
  }
